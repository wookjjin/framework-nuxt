...
## Nuxt 환경에서의 생명주기
지금까지 살펴본 훅들은 모두 순수 Vue의 컴포넌트 생명주기입니다. Nuxt는 서버와 클라이언트를 오가며 실행되는 환경인 만큼, 이를 보완하는 몇 가지 도구를 추가로 제공합니다.

### `onServerPrefetch()`
`onServerPrefetch()`는 Vue가 제공하는 훅이지만, 서버 사이드 렌더링(SSR)이 있는 Nuxt 환경에서 특히 유용합니다. 서버에서 컴포넌트를 렌더링하기 전에 비동기 작업이 끝나기를 기다려야 할 때 사용합니다.
```vue
<script setup>
...
```
::warning
`onServerPrefetch()`는 서버에서만 실행되며, `onBeforeMount()`나 `onMounted()`처럼 SSR 단계에서 호출되지 않는 훅과는 반대되는 성격을 가집니다. 클라이언트와 서버 양쪽에서 데이터를 가져와야 한다면 `useAsyncData()`/`useFetch()`를 사용하는 편이 더 간단합니다.
::

### `callOnce()`
`callOnce()`는 Nuxt 3.9부터 제공되는 유틸리티로, 특정 코드를 서버 렌더링 또는 클라이언트 내비게이션 도중 **한 번만** 실행하고 싶을 때 사용합니다. 서버에서 실행된 결과는 페이로드에 담겨 클라이언트로 전달되므로, 하이드레이션 시 같은 코드가 다시 실행되지 않습니다.
```vue
<script setup>
...
```
- 컴포넌트의 `onMounted()` 안에서 매번 데이터를 새로 가져오는 대신, 앱 생애주기 동안 한 번만 실행되어야 하는 초기화 로직(설정값 로드 등)에 적합합니다.
- 두 번째 인자로 `{ mode: 'navigation' }`을 전달하면 페이지 이동마다 다시 실행되도록 조정할 수 있습니다.

### Nuxt 앱 레벨 훅
컴포넌트 단위가 아니라 애플리케이션 전체의 생명주기(페이지 전환 시작/종료, 앱 마운트 완료 등)를 감지해야 한다면 `nuxtApp.hook()`을 사용합니다. 이는 주로 [플러그인](/docs/advanced/plugins) 안에서 등록합니다.
```ts
...
```
::tip
컴포넌트 생명주기 훅은 "이 컴포넌트의 상태"에 관한 것이고, Nuxt 앱 훅은 "애플리케이션 전체의 흐름"에 관한 것입니다. 필요한 로직의 범위에 따라 적절한 도구를 선택하세요.
::